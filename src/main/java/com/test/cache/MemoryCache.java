...
public class MemoryCache<K, T> {
	...
	public MemoryCache(long memoryTime, final long memoryInterval, int maxItems) {
		this.timeToLive = memoryTime * 1000;

		memoryCacheMap = new LRUMap(maxItems);

		if (timeToLive > 0 && memoryInterval > 0) {

			Thread t = new Thread(new Runnable() {
				public void run() {
					while (true) {
						try {
							Thread.sleep(memoryInterval * 1000);
